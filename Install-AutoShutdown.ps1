...
.PARAMETER StartFromStep
    Resume from a specific step (1-5) if a previous run failed.
    Default: 1
...
Write-Host "  |       Azure VM Autoshutdown - Full Setup             |" -ForegroundColor White
...
function Invoke-Step {
    ...
    Write-Host ""
    Write-Host "  +-----------------------------------------------------+" -ForegroundColor DarkCyan
    Write-Host ("  |  Step {0}/5 - {1,-43}|" -f $Number, $Title)           -ForegroundColor White
    Write-Host "  +-----------------------------------------------------+" -ForegroundColor DarkCyan
    Write-Host ""
...
# -- Step 5 params (runbook + schedule) ---------------------------------------
$step5Params = @{}
...
Invoke-Step 5 "Deploy Runbook + Schedule"     "New-Runbook.ps1"              $step5Params
...
Write-Host "  Schedule           : Daily at $ScheduleTime UTC"        -ForegroundColor White
Write-Host "  WhatIf mode        : ON - no VMs will be stopped yet"   -ForegroundColor Yellow
Write-Host ""
Write-Host "  What to do next:" -ForegroundColor Cyan
Write-Host ""
Write-Host "  1. Tag a test VM:  key = shutdown,  value = true"          -ForegroundColor White
Write-Host "  2. Trigger the runbook manually in Portal, or wait for the schedule" -ForegroundColor White
Write-Host "  3. Check the Job output - your VM should appear in [WHATIF] lines" -ForegroundColor White
...
Write-Host ""
Write-Host "  Onboard a VM  : add tag  shutdown     (any value)"         -ForegroundColor Gray
Write-Host "  Offboard a VM : add tag  donotshutdown  (any value)"       -ForegroundColor Gray
Write-Host ""
