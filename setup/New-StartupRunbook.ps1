<#
.SYNOPSIS
    Uploads the Invoke-AutoStartup runbook and creates the daily schedule.
...
.PARAMETER RunbookPath
    ...
    Default: $PSScriptRoot\..\runbook\Invoke-AutoStartup.ps1

.PARAMETER DisableWhatIf
    If set, the schedule runs with WhatIf=$false (live startups).
...
.EXAMPLE
    # Custom startup time and specific subscriptions
    .\New-StartupRunbook.ps1 -DisableWhatIf -ScheduleTime "06:00" -SubscriptionIds "sub-id-1,sub-id-2"

.NOTES
    Permissions required : Contributor on the Automation Account
    Modules required     : Az.Accounts, Az.Automation
    Run before this      : Import-Modules.ps1, New-Runbook.ps1 (shutdown runbook must exist first)
#>
...
Write-Banner "Deploy Auto-startup Runbook + Schedule"
...
if ($RunbookPath -eq "") {
    $RunbookPath = Join-Path $PSScriptRoot "..\runbook\Invoke-AutoStartup.ps1"
}
...
if (-not $RunbookPath -or -not (Test-Path $RunbookPath)) {
    ...
    Write-Host "  Ensure Invoke-AutoStartup.ps1 is in the runbook/ folder." -ForegroundColor Yellow
    exit 1
}
...
Write-Host ""
if ($whatIfValue) {
    Write-Host "  Mode: WhatIf = TRUE (dry run - no VMs will be started)" -ForegroundColor Yellow
    Write-Host "  Review the first job output, then re-run with -DisableWhatIf to go live." -ForegroundColor Yellow
} else {
    Write-Host "  Mode: WhatIf = FALSE (LIVE - VMs will be started)" -ForegroundColor Green
}
Write-Host ""

# -- Upload runbook ------------------------------------------------------------
Write-Step "Uploading runbook: Invoke-AutoStartup"

$runbookName = "Invoke-AutoStartup"
...
Write-Host "  This will NOT start any VMs." -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    Save-State @{ LastStartupTestJobId = $job.JobId.ToString() }
    ...
    Write-Info "Start manually in Portal: Automation Account → Runbooks → Invoke-AutoStartup → Start"
}

# -- Acceptance criteria -------------------------------------------------------
Invoke-AcceptanceCriteria -StoryId "AutoStartup" -CriteriaNames @(
...
if ($whatIfValue) {
    ...
    Write-Host "  When satisfied, run this script with -DisableWhatIf to enable live startups:" -ForegroundColor Cyan
