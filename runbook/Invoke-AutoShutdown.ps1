<#
.SYNOPSIS
    Autoshutdown runbook for Azure VMs and Azure Local VMs.

.DESCRIPTION
    Shuts down:
      - Classic Azure VMs (Microsoft.Compute/virtualMachines) tagged with "shutdown"
      - Azure Local VMs (Microsoft.AzureStackHCI/virtualMachineInstances) tagged with "shutdown"

    Skips:
      - Any resource tagged with "donotshutdown" (regardless of value)
      - Azure Local Servers (Microsoft.AzureStackHCI/servers) — never touched

    Tag logic:
      - Tag key matching is case-insensitive.
      - A VM is eligible if it has a tag key "shutdown" (any value).
      - A VM is excluded  if it has a tag key "donotshutdown" (any value).
      - "donotshutdown" takes priority over "shutdown" when both are present.
...
.PARAMETER WhatIf
    If set to $true, the script logs what it would do but performs no actual shutdowns.
...
foreach ($subId in $subIds) {
    ...
    foreach ($vm in $classicVMs) {
        ...
        # Priority 1: donotshutdown tag
        if (Has-Tag -Tags $tags -TagKey "donotshutdown") {
            Write-Log "  SKIP — tagged 'donotshutdown'."
            $stats.SkippedDoNotShutdown++
            continue
        }

        # Priority 2: must have shutdown tag
        if (-not (Has-Tag -Tags $tags -TagKey "shutdown")) {
            Write-Log "  SKIP — no 'shutdown' tag."
            $stats.SkippedNoTag++
            continue
        }

        # Priority 3: already deallocated?
        ...
        # Action
        if ($WhatIf) {
            Write-Log "  [WHATIF] Would stop (deallocate) classic VM: $name"
        } else {
            Write-Log "  ACTION — Stopping (deallocating) classic VM: $name ..."
            try {
...
"@ -ErrorAction Stop
    ...
    foreach ($lvm in $localVMs) {
        ...
        # Priority 1: donotshutdown tag
        if (Has-Tag -Tags $tagsHT -TagKey "donotshutdown") {
            Write-Log "  SKIP — tagged 'donotshutdown'."
            $stats.SkippedDoNotShutdown++
            continue
        }
        ...
        # Priority 2: must have shutdown tag
        if (-not (Has-Tag -Tags $tagsHT -TagKey "shutdown")) {
            Write-Log "  SKIP — no 'shutdown' tag."
            $stats.SkippedNoTag++
            continue
        }
        ...
        # Action — Azure Local VMs are stopped via REST API / Az CLI equivalent
        if ($WhatIf) {
            Write-Log "  [WHATIF] Would stop Azure Local VM: $name"
        } else {
            Write-Log "  ACTION — Stopping Azure Local VM: $name ..."
            try {
